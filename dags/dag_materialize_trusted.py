...
MATERIALIZATIONS: list[dict[str, str]] = [
    {
        "view": "superstore_db.vw_trusted_orders",
        "table": "superstore_db.trusted_orders",
        "s3_path": "trusted/orders",
    },
]
...
@dag(
    ...
    catchup=False,
    tags=["superstore", "trusted", "materialization", "trigger-ready"],
)
def materialize_trusted():

    for mat in MATERIALIZATIONS:
        view = mat["view"]
        table = mat["table"]
        s3_path = mat["s3_path"]
        # Suffix for unique task ids (e.g. "trusted_orders")
        ...
        create_table(view=view, table=table, s3_path=s3_path, drop_result=drop_result)
